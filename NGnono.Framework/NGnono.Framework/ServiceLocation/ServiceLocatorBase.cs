...
namespace NGnono.Framework.ServiceLocation
{
    ...
    public abstract class ServiceLocatorBase : IServiceLocator
    {
        ...
        protected ServiceLocatorBase(ILog iLog)
        {
            _log = iLog;
        }
        ...
        public void RegisterSingleton<TService, TType>(string key) where TType : TService
        {
            ...
            DoRegisterSingleton<TService, TType>(key);
        ...
        public virtual TService Resolve<TService>(string key)
        {
            ...
            var instance = DoResolve<TService>(key);
        ...
        {
            OnResolving(new ServiceEventArgs(type.ToString()));
            var instances = DoResolveAll(type);
            OnResolved(new ServiceEventArgs(type.ToString()));
        ...
        {
            return DoIsRegistered(typeToCheck);
        }

        public bool IsRegistered(Type typeToCheck, string nameToCheck)
        {
            return DoIsRegistered(typeToCheck, nameToCheck);
        }
        ...
        protected abstract bool DoIsRegistered(Type typeToCheck);

        protected abstract bool DoIsRegistered(Type typeToCheck, string nameToCheck);
